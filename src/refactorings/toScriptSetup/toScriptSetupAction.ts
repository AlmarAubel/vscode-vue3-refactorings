...
export class ScriptSetupRefactoring implements vscode.CodeActionProvider {
  private createFix(document: vscode.TextDocument, range: vscode.Range): vscode.CodeAction {
    const fix = new vscode.CodeAction(`Refactor to Script setup`, vscode.CodeActionKind.RefactorRewrite);
    ...
    const defineComponentExpression = exportedDeclarations[0].getFirstDescendantByKindOrThrow(
      ts.SyntaxKind.CallExpression
    );

    if (defineComponentExpression.getExpression().getText() !== 'defineComponent') {
      return notFixable;
    }
  ...
  private replaceDefineComponent(callExpression: CallExpression<ts.CallExpression>, sourcefile: SourceFile) {
    const setup = this.getSetupBodyText(callExpression);
    const vueImportDeclaration = sourcefile.getImportDeclaration('vue');

    const definePropsStatement = this.getPropsStatement(callExpression, setup?.propName);

    const defineEmitsStatement = this.getEmitStatement(callExpression, setup?.usesEmits ?? false);
  ...
  private getPropsStatement(callExpression: CallExpression<ts.CallExpression>, varName?: string) {
    const propsStatement = this.getChildOfType(callExpression, ts.SyntaxKind.PropertyAssignment, 'props');
  ...
  private getEmitStatement(callExpression: CallExpression<ts.CallExpression>, usesEmits: boolean) {
    const emitsStatement = this.getChildOfType(callExpression, ts.SyntaxKind.PropertyAssignment, 'emits');
  ...
  private getSetupBodyText(callExpression: CallExpression<ts.CallExpression>): Setup | undefined {
    const setup: Setup = { body: '', usesEmits: false };
    const setupMethod = this.getChildOfType(callExpression, ts.SyntaxKind.MethodDeclaration, 'setup');
    ...
    setupBody.getChildrenOfKind(ts.SyntaxKind.ReturnStatement).forEach(x => x.remove());
  ...
  private addSetupToScript(range: vscode.Range, document: vscode.TextDocument, fix: vscode.CodeAction) {
    const scriptTagRange = new vscode.Range(range.start.line - 1, 0, range.start.line - 1, 999);
  ...
  private getChildOfType<TKind extends ts.SyntaxKind.MethodDeclaration | ts.SyntaxKind.PropertyAssignment>(
  ...
  provideCodeActions(
    ...
    return context.diagnostics
      .filter(diagnostic => diagnostic.code === 'vuer-can-be-converted-to-script-setup')
